...
paths:
  /v1/chat/completions:
    post:
      summary: Create a video with images
      deprecated: false
      description: |-
            ...
            schema:
              ...
              properties:
                ...
                top_p:
                  type: integer
                  description: >-
                    An alternative to temperature sampling, called nucleus sampling, where the model considers the results of tokens with top_p probability mass. So 0.1 means only considering the tokens that make up the first
                    10% probability mass. We generally recommend changing this or `temperature` but not both.
                'n':
                  type: integer
                  description: |-
                ...
                tool_choice:
                  ...
                  properties: {}
                  description: >-
                    Specifies which function the model should call (if any). `none` means the model will not call the function but will generate a message. `auto` means the model can choose between generating a message and calling the function. Using `{"type": "function", "function":
                    {"name": "my_function"}}` forces the model to call this function. If no function exists, the default is `auto`.
                  x-apifox-orders: []
              required:
                - model
                - messages
            ...
            example:
              ...
              messages:
                - role: user
                  content:
                    - type: text
                      text: There is a little bird flying inside.
                    - type: image_url
                      image_url:
                        url: >-
